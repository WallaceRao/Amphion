...
class CodecTrainer(TTSTrainer):
    def __init__(self, args, cfg):
        ...
        with self.accelerator.main_process_first():
            ...
            if self.accelerator.is_main_process:
                ...
                self.logger.info(
                    f"Encoder parameters: {self._count_parameters(self.model['encoder'])/1e6:.2f}M"
                )
                self.logger.info(
                    f"Decoder parameters: {self._count_parameters(self.model['decoder'])/1e6:.2f}M"
                )
                self.logger.info(
    ...
    def get_state_dict(self):
        state_dict = {
            "encoder": self.model["emcoder"].state_dict(),
    ...
    def _train_step(self, batch):
        ...
        gen_loss, train_losses = self._train_gen_step(y, y_, vq_loss, train_losses)
    ...
    def _train_gen_step(self, y, y_, vq_loss, train_losses):
        ...
        # vq loss
        gen_loss += sum(vq_loss) * self.cfg.loss.vq_loss_weight
        train_losses["vq_loss"] = sum(vq_loss)
    ...
    def _valid_step(self, batch):
        ...
        # Discriminator loss, BP, and BP and Grad Updated
        disc_loss, valid_losses = self._valid_disc_step(y, y_, valid_losses)
        total_loss += disc_loss

        # Generator loss, BP, and BP and Grad Updated
        gen_loss, valid_losses = self._valid_gen_step(y, y_, vq_loss, valid_losses)
        total_loss += gen_loss

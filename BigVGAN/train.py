...
def train(rank, a, h):
    ...
    print(
        "Generator params: {} M".format(
            sum(p.numel() for p in generator.parameters()) / 1000000
        )
    )
    ...
    print(
        "Discriminator mpd params: {}".format(
            sum(p.numel() for p in mpd.parameters()) / 1000000
        )
    )
    ...
    print(
        "Discriminator mrd params: {}".format(
            sum(p.numel() for p in mrd.parameters()) / 1000000
        )
    )
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if steps >= a.freeze_step:
                ...
                grad_norm_mpd = torch.nn.utils.clip_grad_norm_(mpd.parameters(), 1000.0)
                grad_norm_mrd = torch.nn.utils.clip_grad_norm_(mrd.parameters(), 1000.0)
                optim_d.step()
            else:
                print(
            ...
            grad_norm_g = torch.nn.utils.clip_grad_norm_(generator.parameters(), 1000.0)
            optim_g.step()
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    # reserve 2-point
                    logger.info(
                        "[Epoch: {}, Step: {}/{}], Gen: {:.2f}, Disc: {:.2f}, Mel-Spec. Error: {:.2f}, s/b: {:.2f}".format(
                            epoch,
                            i,
                            train_steps,
                            loss_gen_all,

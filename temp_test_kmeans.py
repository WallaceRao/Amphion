...
# data_json_path = "Emilia/Emilia-zh+en/Emilia-50k.json.gz"
data_json_path = "/mnt/bn/yuacnwang-speech/dataset/Emilia/emila_json/Emilia-50k.json.gz"
...
class CodecDataset(torch.utils.data.Dataset):
    ...
    def load_cached_paths(self):
        ...
        if self.cache_type == "path":
            ...
            for duration in self.wav_path_index2duration:
                self.index2num_frames.append(duration * 24000)
    ...
    def get_num_frames(self, index):
        ...
        return self.wav_path_index2duration[index] * 24000
    ...
    def __getitem__(self, idx):
        ...
        #     single_feature = dict()
        #     # load speech
        #     speech, sr = librosa.load(buffer, sr=24000)
        ...
        try:
            ...
            return {"speech": speech, "meta": meta, "path": wav_path}
        except:
            return None
...
def build_semantic_model():
    ...
    return semantic_model, output_idx


@torch.no_grad()
def extract_semantic_code(semantic_model, output_idx, input_features, attention_mask):
...
if __name__ == "__main__":
    semantic_model, output_idx = build_semantic_model()
    semantic_model.to("cuda")
    stat_mean_var = torch.load(
        "/mnt/bn/yuacnwang-speech/ckpt/semantic_kmeans/mls_wav2vec2bert_stats.pt"
    )
    semantic_mean = stat_mean_var["mean"]
    semantic_var = stat_mean_var["var"]
    print(semantic_mean, semantic_mean.shape)
    print(semantic_var, semantic_var.shape)
    ...
    mean = 0
    var = torch.zeros(1024)
    x_len = torch.zeros(1024)

    for i in tqdm.tqdm(range(1000010)):
        if i % 1000 == 0:
            print(mean)
            print(var)

        if i == 1000:
            stat = {"mean": mean, "var": var}
            torch.save(
                stat,
                "/mnt/bn/yuacnwang-speech/ckpt/semantic_kmeans/emilia_wav2vec2bert_stats_1k.pt",
            )
        ...
        if i == 10000:
            ...
            torch.save(
                stat,
                "/mnt/bn/yuacnwang-speech/ckpt/semantic_kmeans/emilia_wav2vec2bert_stats_10k.pt",
            )
        ...
        if i == 100000:
            ...
            torch.save(
                stat,
                "/mnt/bn/yuacnwang-speech/ckpt/semantic_kmeans/emilia_wav2vec2bert_stats_100k.pt",
            )
        ...
        if i == 1000000:
            ...
            torch.save(
                stat,
                "/mnt/bn/yuacnwang-speech/ckpt/semantic_kmeans/emilia_wav2vec2bert_stats_1000k.pt",
            )

        idx = idx_list[i]
        return_info = dataset[idx]
        if return_info == None:
            continue
        else:
            try:
                speech = return_info["speech"]
                inputs = processor(speech, sampling_rate=16000)
                input_features = inputs["input_features"][0]
                attention_mask = inputs["attention_mask"][0]
                temp_len = sum(attention_mask)
                feat = extract_semantic_code(
                    semantic_model,
                    output_idx,
                    torch.tensor(input_features).unsqueeze(0).to("cuda"),
                    torch.tensor(attention_mask).unsqueeze(0).to("cuda"),
                )
                feat = feat[0, :temp_len, :]
                temp_mean = feat.mean(dim=0).detach().cpu()
                temp_var = feat.var(dim=0).detach().cpu()
            except:
                continue

            # [ var = \frac{{(var_1 \cdot (len_1 - 1) + var_2 \cdot (len_2 - 1)) + (len_1 \cdot (mean_1 - mean)^2 + len_2 \cdot (mean_2 - mean)^2)}}{{len_1 + len_2 - 1}} ]

            new_mean = (mean * x_len + temp_mean * temp_len) / (x_len + temp_len)
            new_var = (
                var * (x_len - 1)
                + temp_var * (temp_len - 1)
                + x_len * (new_mean - mean) ** 2
                + temp_len * (new_mean - temp_mean) ** 2
            ) / (x_len + temp_len - 1)
    ...
    # libritts_path = "/mnt/bn/yuacnwang-speech/dataset/LibriSpeech/train-clean-100"
    # dataset = []
    # for spk in os.listdir(libritts_path):
    #     for chapter in os.listdir(os.path.join(libritts_path, spk)):
    #         for wav in os.listdir(os.path.join(libritts_path, spk, chapter)):
    #             if wav.endswith(".flac"):
    #                 dataset.append(os.path.join(libritts_path, spk, chapter, wav))
    ...
    #     speech, _ = librosa.load(data_path, sr=16000)
    #     inputs = processor(speech, sampling_rate=16000)
    #     input_features = inputs["input_features"][0]
    #     attention_mask = inputs["attention_mask"][0]
    #     temp_len = sum(attention_mask)
    #     feat = extract_semantic_code(semantic_model, output_idx, torch.tensor(input_features).unsqueeze(0).to("cuda"), torch.tensor(attention_mask).unsqueeze(0).to("cuda"))
    #     feat = feat[0,:temp_len,:]
    #     temp_mean = feat.mean(dim=0).detach().cpu()
    #     temp_var = feat.var(dim=0).detach().cpu()

...
def WarmupLR(
    optimizer, warmup_step=0, down_step=5e4, max_lr=1e-4, min_lr=1e-5, **kwargs
):
...
def WarmupInverseSqrtLR(
    optimizer, warmup_step=32000, down_step=800000, max_lr=4e-4, min_lr=1e-7, **kwargs
):
    # alpha = (max_lr - 1e-5) / warmup_step**2
    alpha = max_lr / warmup_step
    ...
    def lr_lambda(step):
        ...
        s1, s2 = warmup_step, warmup_step + down_step
        if step < s1:
            return init_lr + alpha * step
        else:
            return max_lr * ((warmup_step) ** 0.5) / ((step) ** 0.5)
